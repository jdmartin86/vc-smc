...
class CorrelationCholesky(tfb.Bijector):
  ...
  def _forward_event_shape(self, input_shape):
    if not input_shape.rank:
      return input_shape
    tril_shape = tfb.FillTriangular().forward_event_shape(
        input_shape)
    n = tril_shape[-1]
    if n is not None:
      n += 1
  ...
  def _forward_event_shape_tensor(self, input_shape):
    tril_shape = tfb.FillTriangular().forward_event_shape_tensor(
        input_shape)
    n = tril_shape[-1] + 1
  ...
  def _inverse_event_shape(self, input_shape):
    ...
    return tfb.FillTriangular().inverse_event_shape(y_shape)

  def _inverse_event_shape_tensor(self, input_shape):
    n = input_shape[-1] - 1
    y_shape = tf.concat([input_shape[:-2], [n, n]], axis=-1)
    return tfb.FillTriangular().inverse_event_shape_tensor(y_shape)

  def _forward(self, x):
    x = tf.convert_to_tensor(value=x, name="x")
    batch_shape = x.get_shape().as_list()[-1]

    # Pad zeros on the top row and right column.
    y = tfb.FillTriangular().forward(x)
    rank = tf.rank(y)
    paddings = tf.concat([
  ...
  def _inverse(self, y):
    ...
    # transformation.
    return tfb.FillTriangular().inverse(x[..., 1:, :-1])
