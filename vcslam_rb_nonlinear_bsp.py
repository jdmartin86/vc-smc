...
if __name__ == '__main__':
    ...
    est_dep_params = td_agent.init_dependency_params()
    est_marg_params = td_agent.init_marg_params()
    initial_dependency_params = tf.convert_to_tensor(est_dep_params, tf.float32)
    initial_marginal_params = tf.convert_to_tensor(est_marg_params, tf.float32)
    est_proposal_params = [est_dep_params, est_marg_params]
    ...
    for seed in range(num_seeds):
        ...
        #save the data
        np.savetxt('output/nonlinear/vcsmc_rmse_mean_{}_{}.csv'.format(num_steps,seed), trial_mean_errors, delimiter=',')
        np.savetxt('output/nonlinear/vcsmc_rmse_map_{}_{}.csv'.format(num_steps,seed), trial_map_errors, delimiter=',')
        np.savetxt('output/nonlinear/vcsmc_dep_loss_{}_{}.csv'.format(num_steps,seed), trial_dep_loss, delimiter=',')
        np.savetxt('output/nonlinear/vcsmc_marg_loss_{}_{}.csv'.format(num_steps,seed), trial_marg_loss, delimiter=',')
